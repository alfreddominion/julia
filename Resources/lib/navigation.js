    ...
    if (this.stack.length === 1) {
        if (Ti.Platform.osname === 'android') {
            ...
            var containerWindow = Ti.UI.createWindow({
                navBarHidden : true,
                orientationModes : [Ti.UI.LANDSCAPE_LEFT, Titanium.UI.LANDSCAPE_RIGHT, Ti.UI.PORTRAIT, Titanium.UI.UPSIDE_PORTRAIT],
                backgroundColor : "#fff"
            });
...
NavCon.prototype.pop = function() {
    var windows = this.stack.concat([]);
    (this.navGroup) ? this.navGroup.close(windows[windows.length]) : windows[windows.length].close();
};
